...
# **Introduction**
In this meetup, we will walk you through what Contract testing is. We will explain how PACT works and do a hands-on workshop creating a pact between two micro-services and testing it.
...
If you wish to use your own machine on this playground, please make sure you have all the applications and/or programs listed in the requirements and clone or download this repository.
...
# **Survey Link**
We would love to hear your feedback and make improvements on the way we deliver our playground sessions. Please click the [survey link](https://qabook.typeform.com/to/SFjrAQ) to participate.
...
This class uses port 8111 as default port. It has the method checkEta() that checks estimated time for the station Hammersmith and bus number 613. It then prints out the results. CheckEta() method looks at local host, port, station and the bus number. It then makes an HTTP request and transforms it into JSON and extracts the integer from the result.  
...
```   
17.	You should now see an empty class **WhenComesTheBusTest.java**. Copy the code below and paste it in the vi editor inside the main class. (**Press `i` to enter edit mode in vi**).   
...
In the above code, once the **@PactVerification()** method is run, the mock service will get populated with the information that is set up in the **@Pact** annotated method. In that method we declare the **PactDslJsonBody()**, where we say that we are depending on a string type named ‘station’, string type named ‘nr’ and a integer type named ‘eta’. These are the things that we need from the consumer. We also say that the status should be 200 and the header should be what we have given in **headers.put("Content-Type", "application/json")**.
In the **@PactVerification()** method, we first set the root directory for the pact file to be created in. In the next step then we use the class WhenComesTheBus at the provider port, which means that it won’t use the live application data but the mock/virtual data. Then the method **checkEta** is called with station name and bus number.  
18.	Save the above code by typing  
```
    :wq
 ```
19.	To run the test case WhenComesTheBusTest.java we need to go back to client directory.       
```
    cd $CLIENT
 ```
...
21.	By running the above, the pact test will create a JSON pact file in **/client/target/pacts/**.  
...
cd $BROKER
```  
29.	This folder contains the file **docker-compose.yml** file. Type in
```
docker-compose up
```  
30.	Open a separate tab on your browser and copy paste your linux instance address **<Linux instance address>:8113** which should open up your docker compose like below:  
