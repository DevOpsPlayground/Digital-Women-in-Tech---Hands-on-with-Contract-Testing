...
7. Type **cd $BUSSPRINGBOOT**, Press **Enter**.  
8. Now start the spring boot service by giving the command: **mvn spring-boot:run**.  
...
11.	Copy paste **cd $BUSCOMES**. Click **Enter**.  
12.	Type **vi WhenComesTheBus.java**.  
13.	The class WhenComesTheBus.java will open in the vi editor.  
...
14.Type  ‘ **:q**  ‘ and press Enter to exit the vi editor.  
15.Type or paste **cd $CLIENT**, press **Enter**.  
16.Let’s run the class WhenComesTheBus.java by typing the command **mvn.exec:java-Dexec.mainClass=se.ff.bsc.WhenComesTheBus**.  
18.The class should run and give you the result as below:  
...
19.	Now let’s write our test class WhenComesTheBusTest.java. I have already created a shell class for that. Type in **cd $BUSTEST**, press **Enter**.  
20. Type in **vi WhenComesTheBusTest.java** and then press **Enter**.  
22.	You should now see an empty class **WhenComesTheBusTest.java**. Copy the code below and paste it in the vi editor. (**Press i to make vi in edit mode**).   
...
23.	Save the above code by typing  ‘**:wq** ’ and press **Enter**.   
24.	Run the testcase WhenComesTheBusTest.java by going back to root directory.Type or paste **cd $CLIENT**, press **Enter**.     
25.	 Type ‘ **mvn test** ‘ and press **Enter**. You should see the below result in the terminal.   
![](Images/WhenComesTheBusTestResult.png)  
26.	By running the above pact test will create a json pact file in the location **/client/target/pacts**.  
27.	Once you’re in that directory, type the command **vi BusServiceClient-BusService.json**.  
#**Pact File**    

27.	By entering the command in the above step, you will see the json pact file in vi editor.  
![](Images/PactFile.png)  
...
28.	Type  ‘ **:q**  ‘ and press **Enter** to exit the vi editor. 
...
29.	Type **cd $BROKER**.  
30.	This folder contains the file **docker-compose.yml** file. Type in **docker-compose up**.  
31.	Open a separate tab on your browser and copy paste your linux instance address **<Linus instance address>:8113** which should open up your docker compose like below:  
![](Images/Images/PackBroker.png)  
  
32.	Currently your pack broker does not have your pact file. So let’s publish the pact file onto the pact broker. For this we need to go to the client folder. Type **cd $CLIENT**, press **Enter**.   
33.	Let’s publish the pact file by giving the command **mvn pact:publish**.
34.	Go to the tab that has your docker compose running and refresh the page. You should now see your pact file on it as below.   
![](Images/Images/PublishedPactBroker.png)  
35.	You might have noticed that the pact file is not verified on your broker. In order to verify we need to go to be in the **bs** folder, which has the spring boot application. Lets go up one folder in our current directory and then type ‘ **cd bs**.    
36.	Type in **mvn pact:verify** .  
37.	The above command should verify your pact file on your pact broker. Refresh the tab on which your pack broker is running and it should display the last verified time.   
...
38.	Type **cd $VERIFY** and press **Enter**.  
39.	Type **vi BusStopContractTest.java**.  
...
40.	 Let’s go to main directory verify i.e. **PactTest/verifier**.  
41. 	Type **mvn test** and press **Enter**.  
